...
static int add_range(range_t **ranges, char *lo, int size, const trace_t *trace,
    ...
    if (!IS_ALIGNED(lo)) {
        malloc_error(trace, opnum, "Payload address (%p) not aligned to %d bytes", lo,
                     ALIGNMENT);
        return 0;
    }
    ...
    if ((lo < (char *) mem_heap_lo()) || (lo > (char *) mem_heap_hi()) ||
        ...
        malloc_error(trace, opnum, "Payload (%p:%p) lies outside heap (%p:%p)", lo, hi,
    ...
    for (i = 0; i < trace->num_ops; i++) {
        ...
        switch (trace->ops[i].type) {
            ...
            case REALLOC: /* mm_realloc */
                ...
                if (size > 0) {
                    if (add_range(ranges, newp, size, trace, i, index) == 0) return 0;
                }
    ...
    for (i = 0; i < n; i++) {
        if (stats[i].valid) {
            printf("%2s%4s %5.0f%%%8.0f%10.6f%6.0f %s\n", stats[i].weight != 0 ? "*" : "",
                   ...
                   (stats[i].ops / 1e3) / stats[i].secs, stats[i].filename);
